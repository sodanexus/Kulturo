...
test("le Top distingue une vraie note d'un simple changement de statut", () => {
  assert.equal(isRatingActivityEvent({ event_type: "rated", metadata: { rating: 8 } }), true);
  assert.equal(isRatingActivityEvent({ event_type: "added", metadata: { rating: 7 } }), true);
  assert.equal(isRatingActivityEvent({ event_type: "started", metadata: { rating: 9, date_only: true } }), true);
  assert.equal(isRatingActivityEvent({ event_type: "started", metadata: { from: "wishlist", to: "playing", rating: 9 } }), false);
  assert.equal(isRatingActivityEvent({ event_type: "rated", metadata: { rating: null } }), false);
  assert.equal(isProfileTopEvent({ event_type: "finished", metadata: {} }), true);
});
...
test("le mois automatique ignore un média seulement commencé avec une ancienne note", () => {
  const entries = [
    { id: "started", rating: 9 },
    { id: "rated", rating: 8 },
  ];
  const events = [
    {
      ...
      metadata: { from: "wishlist", to: "playing", rating: 9 },
    },
    {
      media_id: "rated",
      event_type: "rated",
  ...
  const month = latestEventMonth(
    events,
    entries,
    "2026-08",
    entry => Boolean(entry.rating),
    isProfileTopEvent,
  );
...
test("un événement de note laisse le score au badge numérique", () => {
  const result = journalEventPresentation(
    { event_type: "rated", metadata: { rating: 8 } },
    { media_type: "movie" },
  );
  assert.deepEqual(result, { icon: "★", label: "Note enregistrée" });
});
