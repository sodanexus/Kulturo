...
1. Remplacer les fichiers de l’application par le contenu complet de l’archive **3.4.9**, en conservant les valeurs propres au projet dans `config.js`.
...
**Aucune nouvelle requête SQL et aucun redéploiement de fonction Edge ne sont nécessaires pour passer de 3.4.6, 3.4.7 ou 3.4.8 à 3.4.9.** Le schéma et les fonctions Supabase sont identiques.
...
La version affichée dans **Moi** doit être **3.4.9**. Actualiser successivement Bibliothèque, Sorties, Journal et Profil : chaque page doit retrouver ses filtres, sa période et sa position. Ouvrir ensuite plusieurs fiches à la suite, filtrer la Bibliothèque, puis utiliser Modifier et enregistrer : les jaquettes doivent rester stables et la fiche doit revenir avec les nouvelles données. Les [tests de parcours](tests/README.md) permettent de vérifier ce comportement avec des données fictives.
